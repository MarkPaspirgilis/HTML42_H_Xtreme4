...
define('X_TIME_NOW', time());

class XDBJson {

    public static $dir = DIR_ROOT . '_xjsondb/';
    //
    ...
    public static function _log_error($process, $data) {
        self::_log(array(
            ...
            'data' => $data,
            'time' => X_TIME_NOW,
            'datetime' => date('d.m.Y H:i', X_TIME_NOW),
        ));
    }
    ...
    public static function _data($table_name, $data, $insert = true) {
        ...
        //
        if ($insert) {
            $data['insert_date'] = X_TIME_NOW;
            $data['update_date'] = null;
            $data['delete_date'] = null;
        } else {
            $data['update_date'] = X_TIME_NOW;
        }
        //
        $meta = self::get_meta('table', $table_name);
        //
